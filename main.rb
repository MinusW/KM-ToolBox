...
bot.button(custom_id: /^noble\/talent\/submit\/\d+\/\d+$/) do |event|
  event.defer_update
  interaction = Defaults.instance.noble_interactions.get(event.user.id, event.channel.id)
  unless interaction
    event.(content: "Something went wrong, please try again", ephemeral: true)
    next
  end
  p interaction.noble.score
  event.send_message(content:  "#{interaction.noble.score}")
  interaction.submit
end
